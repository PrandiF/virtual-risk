    ...
    <Table className="xl:w-[80%] md:w-[85%] w-[95%] rounded-lg shadow-xl">
      <thead className="bg-orange1 opacity-80 text-white xl:text-lg md:text-base text-[12px] rounded-lg">
        <tr className="text-center rounded-lg">
          ...
          <th className="xl:p-2 p-1 xl:flex hidden justify-center">Vigencia inicio/fin</th>
        </tr>
      </thead>
      <tbody className="bg-white opacity-90 text-black xl:text-lg md:text-base text-[12px] rounded-lg">
        {polizas && !isFilter ? polizas.map((poliza: PolizaProps, i) => (
          <tr key={i} className="text-center">
            ...
            <td className="bg-[#FFA6A6] text-white font-semibold p-2">{poliza.estado}</td>
            <td className="p-2 xl:flex justify-center hidden">
              {new Date(new Date(poliza.vigenciaInicio).setDate(new Date(poliza.vigenciaInicio).getDate() + 1)).toLocaleDateString()} -
              {new Date(new Date(poliza.vigenciaFin).setDate(new Date(poliza.vigenciaFin).getDate() + 1)).toLocaleDateString()}
            </td>
          </tr>
        )) : (
